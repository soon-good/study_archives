...
BooleanExpression 을 and, or 조건으로 나열하여 하나로 묶어서 조건 검사식을 하나의 표현식으로 만들어 조건 검사식을 모듈화할 수 있다. BooleanBuilder를 사용할 때 우리는 BooleanBuilder 내에 BooleanExpression을 주입하여 사용했었다.  

예제로 보험 판촉 행사에서 아래의 조건을 필수 체크해야 하는 경우를 예로 들어보자.
...
이 경우 위의 조건식은 공통적인 조건이 될 수 있다. 보험회사 입장에서 우대 고객조건을 검사하는 조건이기 때문에 한 제품의 특성이 될 수 있다. 여기저기서 공통으로 쓰이는 것이 가능하다.    
