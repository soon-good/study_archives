...
동적 쿼리의 핵심은 Expression을 사용한다는 것이다. queryDsl을 이 정도 쯤까지 공부하고 나니 **"queryDsl의 핵심은 Expression을 사용하는 것~"** 가 되지 않을까 싶다. 따지고 보면 select절에 들어가는 QType 필드들도 모두 Expression을 상속받은 개념에 해당한다. querydsl의 핵심은 Expression인것 같다. 
...
제품의 특정 행사에서 아래의 조건을 필수 체크해야 한다고 해보자.
...
멤버들만 정품 아이패드를 사은품으로 주는 행사를 하기로 했다고 해보자. 그리고 주기적으로 이 멤버들의 보험 가입여부 또한 체크해야 한다고 해보자. 이 경우 조건 검사식이 공통적으로 쓰일 수 있다. (조금 억지스럽긴 하지만 예를 들기 위해... ㅠㅜ). 여기서는 이러한 조건들을 조립해서 BooleanExpression을 반환하는 and 조건 메서드를 만드는 과정을 정리한다.  
